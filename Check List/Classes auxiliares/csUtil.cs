...
namespace Check_List
{
    ...
    #region Constantes
        private static string _CaminhoRegPreferencias = @"SOFTWARE\CheckListWizard\Preferencias";
        private static string _CaminhoRegSubKey = @"SOFTWARE\CheckListWizard";
        private static string _CaminhoRegApp = "CheckListWizard";
    #endregion
    ...
    #region Preferencias de usuário
        ...
        /// <summary>
        /// Carrega o valor salvo no registro em _CaminhoRegPreferencias.
        /// </summary>
        public static object CarregarPreferencia(string p_Campo)
        {
            ...
            try
            {
                RegistryKey PreferenciasUsuario = Registry.CurrentUser.OpenSubKey(_CaminhoRegPreferencias, true);
        ...
        /// <summary>
        /// Salva o valor no registro em _CaminhoRegPreferencias.
        /// </summary>
        public static void SalvarPreferencia(string p_NomeCampo, object p_ValorCampo)
        {
            ...
            try
            {
                ...
                if (PreferenciasUsuario == null)
                {
                    PreferenciasUsuario = Registry.CurrentUser.OpenSubKey(_CaminhoRegSubKey, true);
                    if (PreferenciasUsuario == null)
                    {
                        PreferenciasUsuario = Registry.CurrentUser.OpenSubKey("SOFTWARE", true);
                        PreferenciasUsuario.CreateSubKey(_CaminhoRegApp);
                    }
                    PreferenciasUsuario = Registry.CurrentUser.OpenSubKey(_CaminhoRegPreferencias, true);
                    if (PreferenciasUsuario == null)
                    {
                        PreferenciasUsuario = Registry.CurrentUser.OpenSubKey(_CaminhoRegSubKey, true);
        ...
        public static string[] ListaPreferencias()
        {
            try
            {
                RegistryKey ParametrosUsuario = Registry.CurrentUser.OpenSubKey(_CaminhoRegPreferencias, true);
    ...
    #region Dialogs
        ...
        public static DialogResult EditaParametros(string p_Titulo, string p_Descricao, ref ArrayList p_ListaParametros)
        {
            ...
            lblNomes[0].SetBounds(9, 20, frmParametrosRegistro.ClientSize.Width - 20, 13);
            txtValores[0].SetBounds(12, 36, frmParametrosRegistro.ClientSize.Width - 20, 20);
            txtValores[0].Anchor = AnchorStyles.Right | AnchorStyles.Left | AnchorStyles.Top;

            for (int i = 1; i < p_ListaParametros.Count; i++)
            {
                lblNomes[i].SetBounds(9, txtValores[i - 1].Location.Y + txtValores[i - 1].Height + 5, frmParametrosRegistro.ClientSize.Width - 20, 13);
                txtValores[i].SetBounds(12, lblNomes[i].Location.Y + lblNomes[i].Height + 5, frmParametrosRegistro.ClientSize.Width - 20, 20);
            ...
            _AlturaCampos = lblNomes[0].Height + 5 + txtValores[0].Height + 5;
